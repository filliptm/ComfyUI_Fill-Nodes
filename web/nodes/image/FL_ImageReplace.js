...
app.registerExtension({
    name: "FL.ImageReplace",
    async nodeCreated(node) {
        if (node.comfyClass === "FL_ImageReplace") {
            const MIN_WIDTH = 250;
            const MIN_HEIGHT_WITH_PREVIEW = 500;
            const MIN_HEIGHT_WITHOUT_PREVIEW = 200;
            const PADDING = 10;
            ...
            api.addEventListener("fl_image_replace", (event) => {
