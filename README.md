...
| `FL_RegionPNGOverlay` | Draws one or more source-pixel regions on an image and randomly places PNG overlays from per-region folders. The widget shows and edits each region's width and height for repeatable text/logo box sizing, with optional vignette preview controls. |
