...
| `FL_KsamplerContextWindow` | Video sampler with automatic Wan/LTX/MiniMax H3 frame conversion, compact window/overlap controls, expandable Advanced settings, and model/window progress diagnostics. Preserves the clip duration and native video/audio latents. |
...
**Context Window KSampler:** New nodes use `auto`. Window length snaps down to the connected model's clip grid: Wan `4n+1`, LTX `8n+1`, H3 `17n+5`. Overlap snaps down independently to shared latent positions; H3 uses a bound valid at every token phase. The progress summary reports the resolved profile and latent counts; its tooltip includes requested values, overlap, and the extra causal anchor. Internal window spans can differ from standalone clip lengths. These settings do not resize the complete latent or change H3 shot timing.

Advanced contains the explicit frame profiles, raw `latent_frames`, schedules, blending, FreeNoise, causal handling, temporal axis, and conditioning options. H3 model-aware sampling currently supports `standard_static` with FreeNoise disabled. Batched ignores overlap; stride applies only to uniform schedules. Saved workflows keep their original `video_frames_4n_plus_1` or `latent_frames` interpretation until switched to Auto. Unknown models require an explicit profile or raw latent units.

H3 context sampling also requires the matching core support in `comfy/model_base.py` and `comfy/context_windows.py` for temporal positions, audio boundaries, and packed conditioning masks. Restart ComfyUI and reload the frontend after updating both core and Fill-Nodes.
