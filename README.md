...
| `FL_Audio_Beat_Prompt_Schedule` | Compact node with a full-screen audio/prompt sequencer, Beat This neural beat and downbeat detection, waveform-aligned playback and trimming, confidence-aware markers, true bar shading, beat-grid snapping, and frame-native prompt clips. Outputs the prompt schedule, total frames, cropped master audio, and detected BPM. |
...
For audio-reactive timing, choose or upload audio directly on `FL_Audio_Beat_Prompt_Schedule`, then connect its `prompt_schedule` output to **FL MiniMax H3 Beat Shot Planner** from the dedicated pack. The scheduler also outputs the selected audio crop, total frame count, and detected BPM. Use the existing FL BPM Analyzer and Drum Detector when another node needs beat-position or drum-event JSON. A connected `beat_positions` input remains an optional timing override for existing workflows:
