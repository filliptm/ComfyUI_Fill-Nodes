  ...
  <tr>
    ...
    <td valign="top">
      <ul>
        ...
        <li><a href="#kartel-nodes">📋 Kartel Nodes</a></li>
      </ul>
    </td>
  </tr>
</table>
...
| `FL_CoTracker` | CoTracker-based point/region tracking for video frames with forward/backward track integration, visibility-aware mask synthesis, configurable track radius, and empty-region detection for masked downstream sampling. |
| `FL_CreateShapeImageOnPath` | Renders a batch of frames containing a configurable shape (circle / square / triangle) at JSON-defined per-frame positions, with size, color, opacity, blur and motion-blur controls. Produces both the IMAGE and a MASK. |
...
| `FL_ImageBatchToImageList` | Splits a batched image tensor into a Python list of single-image tensors (one per batch index), the inverse of `FL_ImageListToImageBatch`. |
...
| `FL_ImageOverlay` | Composites an overlay image onto a base using a mask, with 10 alignment presets or custom X/Y offsets, scale presets, 5 blend modes (normal/multiply/screen/overlay/add), opacity, mask invert/feather, and clip-vs-extend-canvas boundary handling. Includes on-node preview toggle. |
| `FL_ImagePicker` | Interactive batch image selector that pauses workflow execution and pops a modal UI so the user can pick which images to keep before the rest of the workflow runs. Configurable timeout. |
...
| `FL_ImageReplace` | Replaces a single image in a batch with a replacement image at the given index, with optional MASK input/output (replacement_mask + per-batch masks) and on-node preview. Handles channel-mismatch, negative-index, and clamping automatically. |
| `FL_ImageToMask` | Extracts a MASK from an IMAGE by selecting a channel: `luminance` (Rec. 601 weights), `red`, `green`, `blue`, or `alpha` (with fallback to a ones-mask if no alpha channel exists). |
| `FL_RegionPNGOverlay` | Draws one or more source-pixel regions on an image and randomly places PNG overlays from per-region folders. The widget shows and edits each region's X/Y position plus width and height for repeatable text/logo box placement, with optional vignette preview controls. |
...
| `FL_LoadImage` | Loads images through a responsive drag-and-drop preview panel backed by the ComfyUI input directory, with Original, Fit, and Fill/crop resize modes plus optional connected width/height overrides. Existing absolute-path workflows remain supported, and transparent images are composited over white. |
...
| `FL_SaveAndDisplayImage` | Saves images to disk via ComfyUI's standard `SaveImage` path **and** passes the image batch through to downstream nodes, with the UI surface preview baked in for inline review. |
...
![🖼️ Image Nodes Example](images/nodes/images_nodes.png)
...
| `FL_DepthBlur` | Depth-of-field blur driven by a depth map: pre-computes N kernels (gaussian / box / disc), builds a per-pixel blur-amount map from focal depth and falloff curve (linear / quadratic / smooth / exponential), then lerps between layers. Supports independent near/far strength, depth invert/remap/smoothing, bokeh brightness boost, optional protect mask, and on-node preview. |
...
| `FL_FloatToInt` | Converts a FLOAT to an INT with a chosen rounding mode: `round` (banker's), `floor`, `ceil`, or `truncate`. |
| `FL_IntToFloat` | Converts an INT to a FLOAT — a trivial type bridge for chaining numeric nodes. |
...
| `FL_RandomShapeGenerator` | Generates batches of training-friendly synthetic images with random shapes (circles, rectangles, triangles, ellipses, lines, polygons, geometric_only) in configurable color modes (random / vibrant / pastel / monochrome / warm / cool / grayscale), with opacity range and background color. Outputs both image and latent. |
...
| `FL_ShowText` | Displays a connected STRING input on the node via a styled DOM widget (header, length badge, monospace body, wheel-to-scroll capture) and passes the same string through to the output. |
| `FL_StringToLoraName` | Bridges a free-form STRING (e.g. dynamic LoRA names generated upstream) to the canonical `loras` dropdown type via `folder_paths.get_filename_list("loras")`, so downstream LoRA-loader nodes accept it. |
...
| `FL_UnloadAllModels` | Triggers ComfyUI's `unload_all_models()`, then runs `gc.collect()` + `torch.cuda.empty_cache()` + `torch.cuda.ipc_collect()` to aggressively free VRAM between heavy workflow stages. |
...
| `FL_KsamplerContextWindow` | Context-window-aware video sampler using ComfyUI's `comfy.context_windows` (static_standard / uniform_standard / uniform_looped / batched) with configurable temporal units (`video_frames_4n_plus_1` or `latent_frames`), fuse method, and per-window progress streaming back to the on-node UI. |
...
| `FL_KsamplerSEG` | Cascade-paint segmented sampler that walks per-region soft masks from `FL_KsamplerSEG_Regions` in a corner-anchored nearest-neighbor order, sampling each region against the in-progress canvas so the model itself does seam blending in feature space. Configurable cascade start corner. |
| `FL_KsamplerSEG_Captioner` | Caption each `SEG_REGIONS` region's padded crop via OpenRouter vision models (GPT-4o-mini, Gemini Flash 1.5, Claude Haiku 4.5) with bounded parallel requests, streaming thumbnails+captions back to an on-node DOM widget as they arrive. Output captions feed the SEG encoder. |
| `FL_KsamplerSEG_Encoder` | Turns per-region captions (from `FL_KsamplerSEG_Captioner`) into per-region CLIP CONDITIONING. Falls back to a shared default positive/negative for regions with no caption. CLIP encodes are cached by exact text to avoid duplicate work. |
| `FL_KsamplerSEG_Regions` | Tessellates an image into N overlapping, feathered Voronoi regions (optional Lloyd relaxation, safe-zone subtraction). Returns soft masks where the SAME mask serves as both the noise mask and the composite blend mask, so coverage is guaranteed and the alpha math normalizes to ~1.0 everywhere. |
| `FL_KsamplerSigma` | Custom sigma-schedule sampler that lets you draw a sigma curve directly on the node and feeds it into a standard KSampler call. Returns model/conditioning/latent/vae/image plus the SIGMAS tensor for inspection or reuse. |
...
| `FL_SamplerStrings` | String generator node with one boolean toggle per sampler reported by `comfy.samplers.KSampler.SAMPLERS` (dynamically populated so new ComfyUI samplers auto-appear) that outputs a comma-separated string of selected sampler names for batch processing workflows. |
| `FL_SchedulerStrings` | String generator node with one boolean toggle per scheduler reported by `comfy.samplers.KSampler.SCHEDULERS` (dynamically populated) that outputs a comma-separated string of selected scheduler names for batch processing workflows. |
...
| `FL_Fal_GPTImage2_Edit` | Edits/composes a variable number of reference images (dynamic sockets driven by `image_count`) against a shared prompt via Fal AI's `openai/gpt-image-2/edit` endpoint. Configurable size presets / custom dimensions, quality (low/medium/high), num_images (1-4), and output format (png/jpeg/webp). |
...
| `FL_Audio_Beat_Prompt_Schedule` | Compact node that opens a full-screen audio and prompt sequencer with drag-and-drop upload, recursive ComfyUI input search, local-folder browsing, smooth playback, source trimming, and a professional single-grid timeline. Offset-controlled beat markers are the sole snapping authority for four-grid prompt creation, movement, resizing, and fades. Middle-mouse viewport panning, Space-bar playback control, and frame-native prompt clips share the waveform-aligned ruler. Outputs the cropped master audio plus the visually aligned beat and drum JSON for the rest of the FL audio ecosystem. |
...
For audio-reactive timing, choose or upload audio directly on `FL_Audio_Beat_Prompt_Schedule`, then connect its `prompt_schedule` output to the H3 timeline node. The scheduler also outputs the selected audio crop, total frame count, and detected BPM. Use the existing FL BPM Analyzer and Drum Detector when another node needs beat-position or drum-event JSON. A connected `beat_positions` input remains an optional timing override for existing workflows:
...
The prompt schedule node loads its waveform as soon as an audio file is selected; queueing is not required. Use the source overview handles to trim, Play for continuous crop-aligned looping, Pause/Stop for transport control, and the main frame ruler to drag or resize prompt clips. Drag the shared center boundary between touching prompts to expand one while shortening the other; the pair stays contiguous and the boundary snaps to the beat grid. Right-click a beat-aligned position on the main timeline to set the audio In or Out point; prompt clips are cropped, rebased, and re-snapped with the audio so they stay aligned to the same musical moments. Prompt boundaries also re-snap automatically after crop, FPS, length, grid-density, beat-offset, cached-analysis, or execution timing changes while preserving touching cuts and valid clip lengths. The crop start and end are always valid snap targets, even when the nearest detected beat falls inside the crop. Double-click a shared prompt boundary to toggle a one-beat crossfade, drag its upper handles to resize the transition, or enter an exact frame count in the inspector. The Beat offset slider previews grid movement live and re-snaps the prompt schedule when the adjustment is committed; the waveform, detected reference markers, and audio remain fixed. FPS and length remain normal node widgets, and length is always a frame count. Analysis is cached by source hash, crop, FPS, and detector settings; offset changes reuse the same detected timing.
...
![☁️ Google Drive Nodes Example](images/nodes/googledrive.png)
...
| Node | Description |
|------|-------------|
| `FL_AnimatedShapePatterns` | Generates batches of animated geometric shape frames using 6 pattern algorithms (grid_wave, grid_ripple, circle_array, orbital, spiral, lissajous) with configurable shape, frame count, dimensions, color, and motion parameters. Outputs both IMAGE and MASK. |
...
| `FL_LTXVMaskedICLoRAGuide` | LTXV-aware IC-LoRA guide node that snaps a guide image+mask to the model's latent cadence at a chosen frame index, exposing `guide_strength` and `denoise_strength` for controlling the IC-LoRA conditioning. Uses the `_INPAINT_GREEN` LTXV inpaint marker. |
| `FL_PathAnimator` | Interactive node that pops a modal path-editor UI for drawing animated motion paths, with selectable interpolation easing (linear, ease-in, ease-out, ease-in-out) and per-frame point output. Integrates with `FL_ImagePicker`-style pause-and-pick session storage. |
| `FL_QwenImageEditStrength` | Per-image strength variant of `TextEncodeQwenImageEditPlus` that lets you control the weight of each of up to 3 reference images (-10.0 to 10.0) and pick the latent combination method (weighted_sum / average / maximum / minimum / multiply). |
| `FL_TimeLine` | 🚧 In-progress timeline scaffolding for video animation with IPAdapter preset selection, dimension control, interpolation modes, and frame/FPS settings. Includes an API endpoint for timeline data but currently returns the input MODEL unchanged. |
...
| `FL_WanVaceToVideoMultiRef` | WAN VACE video-generation node that accepts a control_video + control_masks + multi-frame reference_video (instead of a single reference image), encoding up to N reference frames via VAE and prepending them. Returns positive/negative conditioning, latent, and trim_latent count. |
| `FL_WanVideoBlender` | Cross-fades two WAN-generated video clips over a configurable overlap-frame count. Auto-resizes the second clip to match the first. |
| `FL_WanVideoBlender3Way` | Three-clip cross-fade variant of `FL_WanVideoBlender` for `Input → Continuation → End` workflows. Resizes the second and third clips to match the first. |
| `FL_WanVideoContinue` | Builds a continuation-ready frame buffer by placing overlap frames from the end of an input video at the start and (optionally) overlap frames from the start of an end video at the end, with empty-frame fill level and optional control images / inpaint mask. |
| `FL_WanVideoContinuationBlender` | Specialized blender for clips produced by `FL_WanVideoContinue` — auto-strips the embedded overlap frames before blending so frames aren't duplicated. Use this *after* the continuation generation step. |
| `FL_ZImageControlNetPatch` | Loads a Z-Image / Lumina ControlNet patch with auto-config support and exposes `n_control_layers`, `additional_in_dim`, and `refiner_control` for manual tuning. Marked `EXPERIMENTAL = True`. |

### 📷 Screenshots & Examples

![🚧 Work-in-Progress Nodes Example](images/nodes/wip.png)
...
| Node | Description |
|------|-------------|
| `FL_BulletHellGame` | ⚠️ Placeholder stub — reserved for future bullet-hell game functionality. Currently a no-op. |
| `FL_TetrisGame` | ⚠️ Placeholder stub — reserved for future Tetris game functionality. Currently a no-op. |
...
<details>
<summary><h2 id="-kartel-nodes">📋 Kartel Nodes</h2></summary>

> *Nodes for Kartel job-pipeline integration.*

| Node | Description |
|------|-------------|
| `FL_KartelJobInput` | Decodes a Kartel job payload (job_id, user_id, user_email, app_name, callback_url, params_json) into 25 typed outputs — five job-metadata strings plus four each of strings / ints / floats / bools / images. Image fields are downloaded from `image_url_N` keys inside `params_json`. |
