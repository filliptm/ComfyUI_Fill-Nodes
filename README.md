...
| `FL_Audio_Beat_Prompt_Schedule` | Compact node that opens a full-screen audio and prompt sequencer with drag-and-drop upload, recursive ComfyUI input search, local-folder browsing, smooth playback, source trimming, a stationary cropped waveform and audio reference markers, magnetic beat-grid alignment, and frame-native prompt clips. Outputs the cropped master audio plus the visually aligned beat and drum JSON for the rest of the FL audio ecosystem. |
