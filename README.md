...
| `FL_Audio_Beat_Prompt_Schedule` | Compact node that opens a full-screen audio and prompt sequencer with drag-and-drop upload, recursive ComfyUI input search, local-folder browsing, smooth playback, source trimming, and a professional single-grid timeline. Offset-controlled beat markers, four-grid double-click prompt creation, middle-mouse viewport panning, Space-bar playback control, and frame-native prompt clips share one waveform-aligned ruler. Outputs the cropped master audio plus the visually aligned beat and drum JSON for the rest of the FL audio ecosystem. |
